...
class _SnackManagementScreenState extends State<SnackManagementScreen> {
  ...
  Future<void> _loadData() async {
    ...
    _allDistributions = await DatabaseHelper.getSnackOrders();
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text('No snack distributions yet', style: TextStyle(color: AppTheme.textMuted, fontSize: 14)),
          ],
        ),
      );
    }
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Center(
              child: Text(houseNumber.isNotEmpty ? houseNumber : 'S', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: AppTheme.purpleDark)),
            ),
          ),
              ...
              children: [
                Expanded(
                  child: _buildActionBtn('CONFIRM', Icons.check, Colors.green, () async {
                    await DatabaseHelper.updateSnackOrderStatus(dist['id'], 'approved');
  ...
  Future<void> _createExpenseIfNeeded(Map<String, dynamic> dist) async {
    ...
    final itemName = '[Organizer] Snack: ${snackName.isNotEmpty ? snackName : "Day $dayNum"} - $distName${houseNum.isNotEmpty ? " ($houseNum)" : ""}';
    ...
    try {
      await DatabaseHelper.execute(
        'INSERT INTO expenses (category_id, item_name, amount, paid_to, expense_date, notes, paid_by) VALUES (4, @item, @amount, @paidTo, @date, @notes, @paidBy)',
        substitutionValues: {
          'item': itemName,
          'amount': amount,
          'paidTo': distName,
  ...
  Future<void> _deleteExpenseIfNeeded(Map<String, dynamic> dist) async {
    ...
    try {
      await DatabaseHelper.execute(
        ...
        substitutionValues: {
          'item': isOrg ? 'Snack Distribution - ${info[0]}' : 'Sponsor Distribution - ${info[0]}',
  ...
  void _showDistributionForm() async {
    ...
    List<Map<String, dynamic>> allMembers = await DatabaseHelper.query("SELECT id, house_number, name FROM users WHERE member_type = 'main' ORDER BY house_number");
        ...
        builder: (ctx, setModalState) => Padding(
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                const Text('Snack Distribution', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppTheme.goldPrimary)),
                const SizedBox(height: 6),
                Text('Create a new snack distribution record', style: TextStyle(fontSize: 12, color: AppTheme.textMuted)),
                ...
                const SizedBox(height: 12),
                _buildField(controller: snackNameController, label: 'Snack Name', icon: Icons.restaurant),
                const SizedBox(height: 12),
                SizedBox(
                  height: 50,
                  child: ListView.builder(
                ...
                if (isOrganizerDistribution) ...[
                  ...
                  const SizedBox(height: 4),
                  Text('Organizer expense → Food & Drinks', style: TextStyle(fontSize: 11, color: AppTheme.goldPrimary.withOpacity(0.7), fontStyle: FontStyle.italic)),
                ],
                if (!isOrganizerDistribution) ...[
                  const SizedBox(height: 4),
